...
import { AssetHistory, AssetTransaction, useAssetHistory, useAssetTransactions } from "../betterfrost";
    ...
    <>
      {assetTransactions && (
        <>
          ...
          {assetTransactions && <span className="text-xs dark:text-gray-300">Count: {assetTransactions.length}</span>}
    ...
    <>
      {displayHistory && (
        <>
          ...
          <span className="text-xs dark:text-gray-300">Count: {displayHistory.length}</span>
            ...
            {displayHistory.map((history) => {
              ...
              return (
                <div key={history.tx_hash} className="relative pb-4">
                  {/* Timeline dot */}
                  <div className={`absolute w-4 h-4 rounded-full ${dotColor} -left-6 top-0 border-2 border-white dark:border-gray-900`}></div>
...
export const PolicyPage = () => {
  ...
  const { data: assetTransactions, isLoading: isLoadingTransactions, isError: isErrorTransactions } = useAssetTransactions(policy);
      ...
      <div className="flex-1 flex flex-col sm:flex-row">
        <main className="flex-1 flex flex-col gap-2 dark:text-white">
          ...
          <div className="flex items-center gap-1">
            <span className="text-xs text-gray-500 dark:text-gray-300 font-mono">{policy}</span>
            <ClipboardButton text={policy} className="opacity-70 hover:opacity-100 dark:text-white" />
            <LinkClipboardButton text={policyUrl} className="opacity-70 hover:opacity-100 dark:text-white" />
          </div>
