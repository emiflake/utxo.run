...
export const MiniTransactionCard = ({ txHash }: { txHash: string }) => {
  const { data: txData, isLoading: isLoadingTxData, isError: isErrorTxData } = useTxDataByHash(txHash);
  const { data: txInfo, isLoading: isLoadingTxInfo, isError: isErrorTxInfo } = useTxByHash(txHash);
            ...
            <>
              <div className="flex justify-between items-center text-xs">
                <div className="flex gap-1 items-center">
                  <span className="text-gray-500 dark:text-gray-400">Block {txInfo.block_height}</span>
